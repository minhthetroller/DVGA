...
func TestDataExposure_Easy(t *testing.T) {
	...
	t.Run("store a note and see it in plaintext", func(t *testing.T) {
		doModuleRequest(t, app, "data-exposure", http.MethodPost, "/",
			formBody("action", "add", "title", "TestNote", "value", "supersecretvalue"),
			cookie)
		w := doModuleRequest(t, app, "data-exposure", http.MethodGet, "/", nil, cookie)
		body := w.Body.String()
...
func TestDataExposure_Medium(t *testing.T) {
	...
	t.Run("note added in medium mode is stored and shown as base64", func(t *testing.T) {
		doModuleRequest(t, app, "data-exposure", http.MethodPost, "/",
			formBody("action", "add", "title", "MedNote", "value", plainValue),
			cookie)
		w := doModuleRequest(t, app, "data-exposure", http.MethodGet, "/", nil, cookie)
		body := w.Body.String()
...
func TestDataExposure_Hard(t *testing.T) {
	...
	t.Run("add encrypted note requires password", func(t *testing.T) {
		w := doModuleRequest(t, app, "data-exposure", http.MethodPost, "/",
			formBody("action", "add", "title", "HardNote", "value", "topsecret", "password", "mypassword"),
			cookie)
		body := w.Body.String()
	...
	t.Run("missing password returns error", func(t *testing.T) {
		w := doModuleRequest(t, app, "data-exposure", http.MethodPost, "/",
			formBody("action", "add", "title", "NoPass", "value", "secret"),
			cookie)
	...
	t.Run("decrypt with correct password reveals plaintext", func(t *testing.T) {
		...
		doModuleRequest(t, app, "data-exposure", http.MethodPost, "/",
			formBody("action", "add", "title", "DecryptMe", "value", "DECRYPTED_VALUE", "password", "pass123"),
			cookie)

		// Fetch the page — extract the stored ciphertext for "DecryptMe"
		wGet := doModuleRequest(t, app, "data-exposure", http.MethodGet, "/", nil, cookie)
